...
Furthermore, there were three types of exploratory data analysis performed. The first being a geographic exploration on the frequency of jobs based on location with an interactive mapping. Second, percentages of case status results in the form of a pie chart. Lastly, a bar chart on wages based on different case status results.

# Results 

### Application Usage 

The first tab in the application will ask for optional input via the user in order to produce predicted results based on the kNN model and the linear regression applied. Those results will be displayed on their respective tabs. If the user opts not to include provide that information, they may can explore the data via three methods; separated by three different tabs. Each tab has as set of possible inputs for the data to be subset by, however the user must take caution in apply too many filters or inaccurate filters. If there are too many filters applied there may not be sufficient data or any data at all t display. An example would be filter to only Apple Inc. as employer, then selecting "Construction Worker" as job title. There is not an observation that fits both those criteria which leads to blank results. The specific types of data exploration are detailed further in this section. 

### Linear Regression Summary 
The results of this regression are hidden from the user only the output of predicted salary will be displayed by the application output. 
```{r}
...
Below is a bar chart of the wages compared to the case status results. The user may input different fields of restriction to subset the data they want to see. Below the data is subset by: 
...
When analyzing the data overall the issue is that our regression model did not yield any statistically significant variables. Which is unfortunate but may induce more potential for analysis on this data or type of data. The biggest implication this is that there is no association with prevailing wage and the variables we have imposed. We also see that a majority of the applications are to work in the continental U.S. while there may be some positions not on the continent. 
